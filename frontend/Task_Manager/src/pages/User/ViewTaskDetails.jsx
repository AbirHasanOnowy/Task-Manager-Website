...
const ViewTaskDetails = () => {
    ...
    const updateTodoChecklist = async (index) => {
        ...
        try {
            ...
            if (response.data) {
                // ✅ Update entire task including new status
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-4 mt-4'>
                        <div className='form-card col-span-3'>
                            <div className='flex items-center justify-between'>
                                <h2 className='text-lg md:text-lg font-medium'>{task?.title}</h2>
...
const InfoBox = ({ label, value }) => {
    return (
        <>
            ...
            <p className='text-[12px] md:text-[12px] text-gray-700 mt-0.5'>{value}</p>
        </>
    );
}

const TodoCheckList = ({ text, isCompleted, onChange }) => {
    return (
        <div className='flex items-center gap-3 p-3'>
            <input
                ...
                checked={isCompleted}
                onChange={onChange}
                className='h-4 w-4 text-primary bg-gray-100 border-gray-300 rounded-sm outline-none cursor-pointer'
            />
...
const Attachments = ({ link, index, onClick }) => {
    return (
        <div
            className='flex justify-between bg-gray-50 px-3 py-2 rounded-md mt-2 mb-3 cursor-pointer'
            onClick={onClick}
        >
