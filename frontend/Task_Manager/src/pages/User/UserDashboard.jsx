...
const UserDashboard = () => {
    ...
    const getDashboardData = async () => {
        try {
            const respose = await axiosInstance.get(API_PATHS.TASKS.GET_USER_DASHBOARD_DATA);

            if (respose.data) {
                setDashboardData(respose.data)
                prepareChartData(respose.data || null);
            }
        } catch (error) {
    ...
    useEffect(() => {
        if (!loading && dashboardData) {
            ...
            }, 100); // short delay to allow empty → filled transition
        }
